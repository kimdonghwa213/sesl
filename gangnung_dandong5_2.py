...
Tground = 10 + 273
Tset_heating = 18 + 273  # 난방 설정 온도 (16°C)
Tset_cooling = 25 + 273  # 냉방 설정 온도 (28°C)
...
for k in range(len(hour)):
    ...
    if k > 0:
        # HVAC 시스템의 영향을 포함한 온도 변화 계산 (kW를 W로 변환하여 계산)
        total_heat = (qTotalHouse[k - 1] + qHeating[k - 1] - qCooling[k - 1]) * 1000  # kW → W
        dT = total_heat / (mHouse * cAir * 1000)  # 온도 변화
        Troom[k] = Troom[k - 1] + dT  # 새로운 실내온도

    # 2. 계산된 실내온도로 열부하 계산 (단위를 kW로 통일)
    qVent[k] = ((Tsolair2[k] - Troom[k]) * hv * Agh) / 1000  # W → kW
    qRad[k] = (fracSolarWindow * transGlass * areaHouse * radSolar[k]) / 1000  # W → kW
    qRoof[k] = ((Tsolair2[k] - Troom[k]) * ht * areaRoof * (1 - fr)) / 1000  # W → kW
    qFloor[k] = ((Tground - Troom[k]) * hs * areaHouse) / 1000  # W → kW
    qSideWall[k] = ((Tsolair2[k] - Troom[k]) * ht * areaSideWall * (1 - fr)) / 1000  # W → kW
    qFrontBack[k] = ((Tsolair2[k] - Troom[k]) * ht * areaFrontBack * (1 - fr)) / 1000  # W → kW

    # 총 열부하 계산 (환기부하 포함) - 이미 모든 값이 kW 단위
    qTotalHouse[k] = qRoof[k] + qFloor[k] + qSideWall[k] + qVent[k] + qFrontBack[k] + qRad[k]

    # 3. 난방/냉방 부하 계산 (kW 단위로 변경)
    if Troom[k] < Tset_heating:
        # 난방 필요
        qHeating[k] = ((Tset_heating - Troom[k]) * (mHouse * cAir * 1000)) / 1000  # W → kW
        qCooling[k] = 0
        mode[k] = 1  # 난방 모드
    elif Troom[k] > Tset_cooling:
        # 냉방 필요
        qCooling[k] = ((Troom[k] - Tset_cooling) * (mHouse * cAir * 1000)) / 1000  # W → kW
        qHeating[k] = 0
        mode[k] = 2  # 냉방 모드
    else:
        # 난방/냉방 불필요
...
# 결과 분석 (선택사항)
total_heating = sum(qHeating)  # 총 난방 부하
total_cooling = sum(qCooling)  # 총 냉방 부하
max_heating = max(qHeating)    # 최대 난방 부하
max_cooling = max(qCooling)    # 최대 냉방 부하
...
# 열부하 데이터 플로팅 (단위: kW)
plt.plot(time, qTotalHouse , 'b-', label='Total Heat Load', linewidth=1)
...
# 최저 외기온도 계산 및 출력
min_outdoor_temp_K = min(Tsolair2)
min_outdoor_temp_C = min_outdoor_temp_K - 273.15  # 켈빈 온도를 섭씨로 변환
...
print(qTotalHouse[2317])
print(qTotalHouse[102])
