...
for k in range(len(hour)):
    ...
    qVent[k] = ACH * (Toutdoor[k] - Troom[k]) * mHouse * cAir / 3600
...
# 열부하 데이터 플로팅 (단위: kW)
plt.plot(time, qTotalHouse, 'gray', label='Total Heat Load', alpha=0.5, linewidth=1)
plt.plot(time, -qHeating , 'r-', label='Heating Load', linewidth=1)
plt.plot(time, qCooling , 'b-', label='Cooling Load', linewidth=1)
...
# 난방부하 그래프
ax1.plot(time, qHeating , 'r-', label='Heating Load', linewidth=1)
...
# 냉방부하 그래프
ax2.plot(time, qCooling , 'b-', label='Cooling Load', linewidth=1)
...
# 연간 부하 출력
print(f"Annual Total Heat Load: {sum(abs(qTotalHouse)) :.2f} kWh")
print(f"Annual Heating Load: {sum(qHeating) :.2f} kWh")
print(f"Annual Cooling Load: {sum(qCooling) :.2f} kWh")

# 최대 난방부하 계산 및 출력
max_heating_load = max(qHeating)    # kW 단위로 변환
...
min_room_temp_K = min(Troom)
min_room_temp_C = min_room_temp_K - 273.15  # 켈빈 온도를 섭씨로 변환
min_Troom_hour = np.argmin(Troom) + 1  # 최저 기온이 발생한 시간
print(f"Minimum Greenhouse Temperature: {min_room_temp_C:.2f}°C (Hour {min_Troom_hour})")
...
print(f"난방부하: {qHeating[max_heating_idx] :.2f}kW")
...
print(f"난방부하: {qHeating[min_temp_idx] :.2f}kW")
