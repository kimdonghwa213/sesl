...
mHouse = areaHouse * heightHouse * nFloor * rhoAir # [kg]
ht = 5.7 # 열 관류율 [㎉/㎡·h·℃]
...
for k in range(len(hour)):
    ...
    qRad[k] = fracSolarWindow * transGlass * areaHouse * radSolar[k] #[W]
    ...
    qTotalHouse[k] = qRoof[k] + qFloor[k] + qSideWall[k] + qVent[k] + qFrontBack[k] + qRad[k] #[W]

    # 난방/냉방 부하 계산
    if Troom[k] < Tset_heating:
        qHeating[k] = -qTotalHouse[k] if qTotalHouse[k] < 0 else 0  # 열손실일 때만 난방
        qCooling[k] = 0
    elif Troom[k] > Tset_cooling:
        qCooling[k] = qTotalHouse[k] if qTotalHouse[k] > 0 else 0  # 열획득일 때만 냉방
        qHeating[k] = 0
    else:
        qHeating[k] = 0
        qCooling[k] = 0
